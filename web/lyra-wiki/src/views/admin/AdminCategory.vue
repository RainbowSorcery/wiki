  ...
  <a-layout-content :style="{ background: '#fff', padding: '24px', margin: 0, minHeight: '280px' }">
    <p>
      <a-space size="small">
        ...
        <a-button type="primary" @click="queryCategoryCondtionList">查询</a-button>
        <a-button type="primary" @click="addCategory">新增电子书</a-button>
      </a-space>

    </p>

    <a-table :columns="columns" :data-source="Category"  :row-key="setRowKey" 
      :pagination="false"
    ...
    >
        ...
        <template #action="{text}" >
          <a-space size="small">
            <a-button type="primary" @click="regeditCategory(text)">编辑</a-button>
    ...
    <a-modal v-model:visible="visible" title="Basic Modal" @ok="handleOk">
      <a-form :model="sumbitCategory" :label-col="{ span: 4 }" :wrapper-col="{ span: 14 }">
        ...
        <a-form-item label="父分类">
          <a-select
              ...
              >
                ...
                <a-select-option v-for="category in parentCategory" :key="category.id" :value="category.id" :disabled="sumbitCategory.id == category.id">{{category.name}}</a-select-option>
...
export default defineComponent({
  ...
  setup() {
    ...
    const regeditCategory = (parm: CategoryObject) => {
      ...
      visible.value = true
      sumbitType = 'update'
      axios.get("/category/getCategoryById?id=" + toRaw(parm).id).then((response) => {
